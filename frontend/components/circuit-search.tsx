...
// Header search for circuits. Deliberately a close mirror of
// components/constructor-search.tsx — same portal dropdown, same keyboard
// handling, same visual weight — so the two profile pages feel like one
// product rather than two.
//
// Difference from the constructor version: the secondary line shows
// location + country, since "Silverstone" and "Yas Marina" mean nothing
// without a place, whereas a team name stands alone.
...
type Props = {
  /** Currently selected circuit id (controls the closed-state label). */
  selectedId: number | null
  onSelect: (id: number) => void
}

export function CircuitSearch({ selectedId, onSelect }: Props) {
  ...
  // Load the full list once; filtering happens client-side.
  useEffect(() => {
    fetch('/api/circuits')
      .then((r) => (r.ok ? r.json() : { circuits: [] }))
      .then((j) => setCircuits(j.circuits ?? []))
      .catch(() => setCircuits([]))
  }, [])
  ...
  const results = useMemo(() => {
    ...
    if (nq === '') return circuits
    return circuits.filter(
      (c) =>
        norm(c.name).includes(nq) ||
        norm(c.location ?? '').includes(nq) ||
        norm(c.country ?? '').includes(nq)
    )
  }, [circuits, q])
  ...
  const placeholder = selected ? selected.name : 'Search circuit…'
        ...
        createPortal(
          <>
            ...
            <style jsx global>{`
              ...
              .circuit-scroll::-webkit-scrollbar-track {
            ...
            >
              ...
              {/* Results */}
              <ul role="listbox" className="circuit-scroll max-h-[320px] overflow-y-auto p-1">
                {results.length === 0 && (
                  <li
