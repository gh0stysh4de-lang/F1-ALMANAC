...
// Normalize for accent-insensitive matching (e.g. "Räikkönen" -> "raikkonen").
...
type Props = {
  /** Currently selected driver id (controls the closed-state label). */
  selectedId: number | null
  onSelect: (id: number) => void
}

export function DriverSearch({ selectedId, onSelect }: Props) {
  ...
  const results = useMemo(() => {
    ...
    if (nq === '') return drivers
    return drivers.filter((d) => {
      const hay = norm(`${d.name} ${d.code ?? ''}`)
  ...
  const placeholder = selected ? selected.name : 'Search driver…'
        ...
        createPortal(
          <>
            ...
            <style jsx global>{`
              ...
              .driver-scroll::-webkit-scrollbar {
                width: 5px;
              }
              .driver-scroll::-webkit-scrollbar-track {
                background: transparent;
              }
              .driver-scroll::-webkit-scrollbar-thumb {
                background: rgba(255, 255, 255, 0.16);
                border-radius: 999px;
              }
              .driver-scroll::-webkit-scrollbar-thumb:hover {
            ...
            >
              ...
              {/* Results */}
              <ul role="listbox" className="driver-scroll max-h-[320px] overflow-y-auto p-1">
                {results.length === 0 && (
                  <li
                  ...
                  <li key={d.id}>
                    <button
                      ...
                      style={{
                        ...
                        color: d.id === selectedId ? HEADER_PRIMARY : 'rgba(199, 197, 208, 0.78)',
                      }}
                    >
