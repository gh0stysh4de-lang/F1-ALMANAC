...
// Cache the directory listing in production: the folder only changes on
// redeploy, and re-reading it per request would be wasteful. In development
// we skip the cache entirely so that dropping in or replacing a logo file
// shows up on the next refresh without restarting the dev server.
let cachedFiles: string[] | null = null;

async function listLogoFiles(): Promise<string[]> {
  const isDev = process.env.NODE_ENV !== "production";
  if (!isDev && cachedFiles !== null) return cachedFiles;
  try {
    const files = await fs.readdir(LOGO_DIR);
    if (!isDev) cachedFiles = files;
    return files;
  } catch {
    // Directory missing entirely — treat as "no logos available".
    if (!isDev) cachedFiles = [];
    return [];
  }
}
  ...
  try {
    ...
    return new NextResponse(new Uint8Array(data), {
      headers: {
        ...
        // Logos are static in production, so cache hard there. In development
        // tell the browser not to cache, otherwise a replaced file keeps
        // showing the old image until a hard refresh.
        "Cache-Control": isDev
